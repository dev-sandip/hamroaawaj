...
class ReportController {
  ...
  public static FetchReportWithoutLabel = async (
    req: Request,
    res: Response
  ) => {
    try {
      const reports = await Report.find();
      const reportsWithoutLabels = reports.filter(
  ...
  public static vote = async (req: Request, res: Response) => {
    try {
      ...
      validReport.upvote = validReport.upvote.filter(
        (upvoterId) => upvoterId !== userId
      );

      validReport.downvote = validReport.downvote.filter(
        (downvoterId) => downvoterId !== userId
      );
